...
import { SelectSingleFile, ActionPdfToWord, OpenFileFolder, GetSavePath } from '../../wailsjs/go/main/App';
import { motion, AnimatePresence } from 'framer-motion';
import { FileText, Upload, Loader2, CheckCircle2, FolderOpen, RefreshCw, ArrowRight } from 'lucide-react';

const hoverDiv = (color: string, bg: string) => ({
  ...
  onMouseLeave: (e: React.MouseEvent<HTMLDivElement>) => {
    e.currentTarget.style.borderColor = color;
    e.currentTarget.style.background = bg.replace('0.04', '0.03');
  },
});

const hoverBtn = (bgHover: string, bgDefault: string) => ({
  onMouseEnter: (e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.background = bgHover; },
  onMouseLeave: (e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.background = bgDefault; },
});
...
export const PdfToWord = () => {
  ...
  const handleSelect = async () => {
    const selected = await SelectSingleFile('PDF Files', '*.pdf');
    if (selected) { setPath(selected); setResultPath(''); }
  };

  const handleConvert = async () => {
    if (!path) return;
    const defaultName = path.split('\\').pop()?.replace(/\.[^/.]+$/, '') + '.docx';
    const savePath = await GetSavePath(defaultName, '*.docx');
    if (!savePath) return;
    setLoading(true);
    const result = await ActionPdfToWord(path, savePath);
  ...
  const accent = '#e05c4b';
  const accentBg = 'rgba(224,92,75,0.1)';
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '28px 32px 24px', borderBottom: '1px solid #f1f3f5', display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div>
          <h2 style={{ fontSize: 17, fontWeight: 700, color: '#111', margin: 0, letterSpacing: '-0.3px' }}>PDF sang Word</h2>
          <p style={{ fontSize: 13, color: '#8b949e', margin: '3px 0 0', lineHeight: 1.4 }}>Chuyển đổi file PDF sang định dạng .docx có thể chỉnh sửa</p>
        </div>
      </div>
      ...
      <div style={{ padding: '28px 32px 32px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div
          ...
          style={{ border: `2px dashed ${path ? accent : '#d1d5db'}`, borderRadius: 14, padding: '32px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, cursor: 'pointer', background: path ? 'rgba(224,92,75,0.03)' : '#fafafa', transition: 'all 0.18s ease' }}
          ...
          onMouseLeave={(e: React.MouseEvent<HTMLDivElement>) => { e.currentTarget.style.borderColor = path ? accent : '#d1d5db'; e.currentTarget.style.background = path ? 'rgba(224,92,75,0.03)' : '#fafafa'; }}
        >
          ...
          {path ? (
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#374151' }}>Nhấn để chọn file PDF</div>
              <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 4 }}>Hỗ trợ PDF một trang hoặc nhiều trang</div>
            </div>
          )}
        </div>
        ...
        <AnimatePresence mode="wait">
          {!resultPath ? (
            <motion.button
              ...
              onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { if (path && !loading) e.currentTarget.style.background = '#c94d3c'; }}
            ...
            <motion.div key="result" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ padding: '14px 20px', borderRadius: 12, background: 'rgba(22,163,74,0.07)', border: '1px solid rgba(22,163,74,0.2)', display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: '#15803d' }}>Chuyển đổi thành công!</div>
                  <div style={{ fontSize: 11, color: '#4ade80', marginTop: 1, opacity: 0.8 }}>File .docx đã được lưu</div>
                </div>
              </div>
